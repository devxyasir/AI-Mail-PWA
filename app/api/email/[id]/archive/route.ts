 ...
 *
 * Archives a message using its composite ID.
 */
export async function POST(
  ...
  try {
    ...
    // 3. Archive via provider
    const provider = createProvider(decryptedAccount);
    await provider.archiveMessage(providerMessageId);

    return NextResponse.json({ success: true });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    console.error(`[API] Archive error (${id}):`, error);
    ...
    return NextResponse.json(
      { error: 'Failed to archive message', details: message },
      { status: 500 }
    );
  }
}
